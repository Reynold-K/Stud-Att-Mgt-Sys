...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Replace the existing INSERT code with this:
if (!empty($_POST['m_id']) && !empty($_POST['m_subject']) && !empty($_POST['m_date']) && !empty($_POST['m_day']) && !empty($_POST['m_time'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "INSERT INTO `present_table`(`semester`, `id`, `subject`, `date`, `day`, `time`) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("isssss", $semester, $id, $subject, $new_date, $day, $time);

        if ($stmt->execute()) {
            echo "<script>";
            echo "alert('Student attendance recorded successfully!')";
            echo "</script>";
        } else {
            echo "<script>";
            echo "alert('Error in inserting data: " . $stmt->error . "')";
            echo "</script>";
        }
        $stmt->close();
    }
