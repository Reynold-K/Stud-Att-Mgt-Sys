...
// Validate input
if (!$input || !isset($input['response']) || !isset($input['response']['clientDataJSON'])) {
    echo json_encode(['success' => false, 'message' => 'Invalid WebAuthn response: Missing clientDataJSON']);
    exit();
}
...
if (!$clientData || !isset($clientData['challenge'])) {
    echo json_encode(['success' => false, 'message' => 'Invalid WebAuthn response: Missing or invalid challenge']);
    exit();
}
